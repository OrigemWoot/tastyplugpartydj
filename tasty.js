...
(function($) {
    ...
    function custEmotes(txt, message) {
        var keys = Object.keys(emotes),
            html = txt.html();
        if (!keys.length || typeof html != 'string') return;
        ...
        for (var i = 0; i < em.length; i++) {
            ...
            if (keys.indexOf(emlow) != -1) {
                var img = new Image();
                img.dataset.emoteName = emlow;
                img.onload = function() {
                    this.className = "tp-emote";
                    this.title = emlow;
                    html = html.replace(":" + this.dataset.emoteName + ":", this.outerHTML);
                    var chat = $('#chat-messages'),
                        d = chat.scrollTop() > chat[0].scrollHeight - chat.height() - 28;
                    txt.html(html);
                    if (d) chat.scrollTop(chat[0].scrollHeight);
                };
                img.src = emotes[emlow];
            }
        }
    }
